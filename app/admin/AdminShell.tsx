      ...
      <div className="flex min-h-screen">
        <aside className="hidden w-72 shrink-0 border-r border-slate-200 bg-white/95 px-4 py-5 shadow-sm lg:block">
            ...
            {visibleNavItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={[ 'mt-1 text-xs', active ? 'text-blue-600' : 'text-slate-500' ].join(' ')}>
                    {item.description}
                  </div>
                </Link>
              )
          ...
          <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
            ...
            <button onClick={handleLogout} className="mt-4 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-bold text-slate-700 hover:bg-slate-100">
              ログアウト
            </button>
          </div>
        </aside>
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 px-4 py-3 shadow-sm backdrop-blur lg:hidden">
            <div className="mb-3 flex items-start justify-between gap-3">
              <div>
                <p className="text-xs font-semibold text-blue-600">NCI Management</p>
                <h1 className="text-lg font-bold text-slate-900">管理システム</h1>
              </div>
              <button onClick={handleLogout} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-bold text-slate-700">
                ログアウト
              </button>
            </div>
          ...
          <header className="hidden border-b border-slate-200 bg-white/85 px-8 py-4 backdrop-blur lg:block">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-xs font-semibold text-blue-600">Admin Console</p>
                <h2 className="text-xl font-bold text-slate-900">NCI 管理メニュー</h2>
              </div>
              <div className="rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-xs font-semibold text-slate-600">
                {currentRole?.role === 'admin' ? '全支店管理者' : currentRole?.role === 'manager' ? '支店管理者' : '営業担当者'}
              </div>
            </div>
          </header>
