...
function init(db) {
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_branch_master_sort ON branch_master(sort_order, name);
...
function seedBranchMaster(db) {
  const exists = db.prepare('SELECT COUNT(*) AS c FROM branch_master').get().c;
  if (exists > 0) return;

  const names = db.prepare(`
    SELECT DISTINCT branch AS name FROM staff WHERE branch IS NOT NULL AND branch<>''
    UNION
    SELECT DISTINCT branch AS name FROM jin_date_rows WHERE branch IS NOT NULL AND branch<>''
    ORDER BY name
  `).all();
...
export function listBranches(db = getDb()) {
  return db
    .prepare('SELECT name FROM branch_master ORDER BY sort_order, name')
    .all()
    .map((row) => row.name);
}
